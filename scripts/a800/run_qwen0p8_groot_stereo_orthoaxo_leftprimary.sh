#!/bin/bash
# Qwen3.5-0.8B + GR00T, stereo + axonometric orthogonal cache.
...
run_id=${RUN_ID:-qwen0p8_groot_stereo_orthoaxo_leftprimary_fromscratch_fullft_eff128_maskfix_30k}
...
ORTHO_PROMPT=${ORTHO_PROMPT:-"The visual input contains three images: the primary robot camera, the left stereo camera, and an axonometric view rendered from the stereo tabletop point cloud. Your task is {instruction}. To identify the key objects for your task. Locate their bounding boxes in [x1,y1,x2,y2] format."}
...
case "${run_id}" in
  ...
  *stereo_orthoaxo*) { [ "${CAM_ROPE}" = "0" ] && [ "${CAM_BRANCH}" = "0" ]; } || { echo "[guard] orthoaxo arm needs CAM_ROPE=0 CAM_BRANCH=0"; exit 3; } ;;
...
echo "[launch] STEREO+ORTHO_AXO QwenGR00T | cam_rope=${CAM_ROPE_BOOL} cam_branch=${CAM_BRANCH_BOOL} | BS=$BS x $NUM_PROCESSES x GA${DS_GA} = eff_$((BS*NUM_PROCESSES*DS_GA)) | MAX_STEPS=$MAX_STEPS save=$SAVE_INTERVAL warmup=$WARMUP_STEPS | run_id=$run_id | ortho_cache=${ORTHO_CACHE_DIR}"
