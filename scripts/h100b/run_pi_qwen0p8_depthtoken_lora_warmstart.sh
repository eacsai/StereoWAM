...
run_id=${RUN_ID:-pi_qwen0p8_depthtoken_lora_warmstart_$(date +%m%d)}
PORT=${PORT:-29733}
FREEZE_MODULES=${FREEZE_MODULES:-}
RUN_DESC="DepthToken FFS: VLM LoRA warm-start, train action head + depth projector"
...
echo "[launch] FREEZE_MODULES='${FREEZE_MODULES}' (PEFT freezes base VLM; adapters stay trainable)"
...
CUDA_VISIBLE_DEVICES=${GPUS} ${CONDA_VENV}/accelerate launch \
  ...
  --framework.name "QwenPIDepthTokenLoRAFFS" \
  ...
  --framework.ffs_depth_token.primary_idx 0 \
  --framework.ffs_depth_token.right_view_idx 1 \
  --framework.ffs_depth_token.num_cameras 2 \
  --framework.ffs_depth_token.num_depth_tokens 16 \
  --framework.ffs_depth_token.pool_hw 4 \
  --framework.ffs_depth_token.vlm_lora true \
  --framework.ffs_depth_token.vlm_lora_r 16 \
  --framework.ffs_depth_token.vlm_lora_alpha 32 \
  --framework.ffs_depth_token.vlm_lora_dropout 0.0 \
  --framework.ffs_depth_token.vlm_lora_target_modules "q_proj,k_proj,v_proj,o_proj" \
  ...
  --trainer.learning_rate.qwen_vl_interface 1e-4 \
  --trainer.learning_rate.depth_token_projector 1e-4 \
  ...
  --run_id ${run_id} \
  --wandb_project starVLA_FFS_DepthToken \
