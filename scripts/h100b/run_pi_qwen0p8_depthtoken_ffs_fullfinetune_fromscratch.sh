...
run_id=${RUN_ID:-pi_qwen0p8_depthtoken_ffs_fullfinetune_fromscratch_$(date +%m%d)}
PORT=${PORT:-29732}
FREEZE_MODULES=${FREEZE_MODULES:-}
# HIGH-1 (codex review): C is a STRESS run, NOT a clean attribution point for depth tokens
# (it mixes from-scratch + full VLM finetune + depth). interleave_self_attention set to
# true to match Run A (remove the A-vs-C action-arch confound). Real depth-token attribution
# comes from Run A (frozen) and LoRA-B.
RUN_DESC="DepthToken FFS STRESS run: full finetune from base Qwen3.5-0.8B, no warm-start ckpt"
...
CUDA_VISIBLE_DEVICES=${GPUS} ${CONDA_VENV}/accelerate launch \
  ...
  --framework.name "QwenPIDepthTokenFFS" \
