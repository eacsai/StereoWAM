#!/bin/bash
# Qwen3.5-0.8B + GR00T + FFS #7 ControlVLA parallel K/V branch.
# Arms:
#   fromscratch: RUN_ID=qwen3p5_0p8b_ffs_controlvla_branch_fromscratch_30k PRETRAINED_CKPT= TRAIN_ONLY=
#   warmstartB:  RUN_ID=qwen3p5_0p8b_ffs_controlvla_branch_warmstartB_30k PRETRAINED_CKPT=<B> TRAIN_ONLY=<branch allowlist>
set -e
...
FRAMEWORK=${FRAMEWORK:-QwenGR00T_ControlVLAFFS}
B_CKPT=${B_CKPT:-}
...
run_id=${RUN_ID:-qwen3p5_0p8b_ffs_controlvla_branch_fromscratch_30k}
...
TRAIN_ONLY_DEFAULT="ffs_pos_emb,.attn1.to_k_z.,.attn1.to_v_z."
...
[ "${FRAMEWORK}" = "QwenGR00T_ControlVLAFFS" ] || {
  echo "[guard] this launcher requires FRAMEWORK=QwenGR00T_ControlVLAFFS, got '${FRAMEWORK}'"; exit 3;
}
[ "${CAM_ROPE}" = "0" ] || { echo "[guard] ControlVLA arms are defined with CAM_ROPE=0, got '${CAM_ROPE}'"; exit 3; }
[ "${CAM_BRANCH}" = "0" ] || { echo "[guard] ControlVLA arms are defined with CAM_BRANCH=0, got '${CAM_BRANCH}'"; exit 3; }
...
case "${run_id}" in
  ...
  qwen3p5_0p8b_ffs_controlvla_branch_warmstartB_30k)
    ...
    ;;
  *)
    echo "[guard] run_id must be one of the two ControlVLA arms, got '${run_id}'"; exit 3 ;;
esac
...
echo "[launch] ${FRAMEWORK} + Qwen3.5-0.8B + GR00T ControlVLA K/V branch"
...
CUDA_VISIBLE_DEVICES=${GPUS} ${CONDA_VENV}/accelerate launch \
  ...
  --framework.ffs_controlvla.gru_hidden_dim 16 \
  --framework.ffs_controlvla.ffs_image_size 256 \
  --framework.ffs_controlvla.ffs_pool_size 8 \
  --framework.ffs_controlvla.num_cameras 2 \
  --framework.ffs_controlvla.left_ref_idx 1 \
  --framework.ffs_controlvla.primary_view_idx 0 \
  --framework.ffs_controlvla.inject_cam_id 1 \
  ...
  --run_id ${run_id} \
  --wandb_project starVLA_Qwen0p8_GR00T_ControlVLA \
