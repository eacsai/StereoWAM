#!/bin/bash
run_id=${RUN_ID:-pi_qwen0p8_vlmcontrolnet_ffs_frozenhead_warmstart_$(date +%m%d)}
PORT=${PORT:-29723}
FREEZE_MODULES=${FREEZE_MODULES:-qwen_vl_interface,action_model}
RUN_DESC="Run3 pure ControlNet: frozen VLM + action head, train control branch only"
...
inner_json=$(grep -oE 'deepspeed_config_file: *"?[^"]+' "$DS_CONFIG" | sed -E 's|.*: *"?||')
...
CUDA_VISIBLE_DEVICES=${GPUS} ${CONDA_VENV}/accelerate launch \
  ...
  --framework.name "QwenPIVLMControlNetFFS" \
  ...
  --framework.ffs_vlm_controlnet.ffs_scale 0 \
  --framework.ffs_vlm_controlnet.ffs_image_size 256 \
  --framework.ffs_vlm_controlnet.primary_idx 0 \
  --framework.ffs_vlm_controlnet.right_view_idx 1 \
  ...
  --run_id ${run_id} \
  --wandb_project starVLA_VLMControlNet_FFS \
