#!/bin/bash
run_id=${RUN_ID:-pi_qwen0p8_vlmcontrolnet_ffs_trainhead_warmstart_$(date +%m%d)}
PORT=${PORT:-29722}
FREEZE_MODULES=${FREEZE_MODULES:-qwen_vl_interface}
RUN_DESC="Run2 treatment: VLM ControlNet FFS, frozen VLM, train action head + control branch"
