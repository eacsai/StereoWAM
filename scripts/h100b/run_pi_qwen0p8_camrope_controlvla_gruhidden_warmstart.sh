#!/bin/bash
# H100b launcher: PI + Qwen3.5-0.8B + cam_rope + FFS ControlVLA-style branch,
# injecting the FFS GRU-HIDDEN state net[0] (post-cost-volume, disparity-aware)
# instead of the monocular backbone feature pyramid.
#
# vs run_pi_qwen0p8_camrope_controlvla_branch_fromscratch.sh (the backbone variant):
#   * adds --framework.ffs_controlvla.ffs_feature_source gru_hidden
#     (default "backbone" = concat[left,right] monocular Feature pyramid, NO disparity;
#      "gru_hidden" = FFS net[0] after cost volume + 8 GRU refinement iters = real stereo).
#   * fromscratch (NO warm-start), same eff_batch 96 / 30k / data / FFS ckpt.
#   * ffs_token_dim becomes hidden_dims[0]=128 (single fused map, no L+R concat).
#
# Motivation: 2026-05-29 audit found the backbone-feature variants inject MONOCULAR
# features (FoundationStereo's Feature module is a per-view 2D CNN run before the cost
# volume), which explains why stereo == mono on libero_goal. net[0] carries the actual
# left-right matching / disparity signal. See memory project_ffs_features_are_monocular.
set -e
...
# codex round-1 MED#3: pin FFS ckpt hash (same file on h100b /mnt/data and 4090d /data).
ffs_sha256=98b5a9acf39fbfa795025de8cea95ce123daa40f6b6234d719167751024cf692
pretrained_ckpt=/mnt/data/wangqiwei/wangqiwei/starVLA/playground/Checkpoints/goal_phase3b_camrope_0523_baseline_for_initdisp_init/checkpoints/steps_30000_pytorch_model.pt

run_root_dir=./playground/Checkpoints
run_id=${RUN_ID:-pi_qwen0p8_camrope_controlvla_gruhidden_warmstart_h100b_$(date +%m%d)}

BS=${BS:-24}                                     # eff_batch = BS x NUM_PROCESSES x GA(2) = 96
MAX_STEPS=${MAX_STEPS:-30000}
GPUS=${GPUS:-0,1}
NUM_PROCESSES=${NUM_PROCESSES:-2}
PORT=${PORT:-29705}                              # distinct from controlvla(29701)/from_baseline96(29703)
...
# Symlink stereo dataset (fail-closed, same as the backbone variant)
