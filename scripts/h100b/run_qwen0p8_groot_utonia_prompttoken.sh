...
[ "${CAM_BRANCH}" = "0" ] || { echo "[guard] Utonia prompt-token run is incompatible with CAM_BRANCH=1"; exit 3; }
...
[ -n "${UTONIA_CACHE_DIR}" ] || { echo "[guard] prompt-token comparison must use cached Utonia; set UTONIA_CACHE_DIR"; exit 3; }
[ "${MAX_STEPS}" = "30000" ] || { echo "[guard] fair comparison uses MAX_STEPS=30000, got '${MAX_STEPS}'"; exit 3; }
[ "${SAVE_INTERVAL}" = "10000" ] || { echo "[guard] fair comparison uses SAVE_INTERVAL=10000, got '${SAVE_INTERVAL}'"; exit 3; }
...
if [ "${SKIP_UTONIA_PROMPTTOKEN_SMOKE:-0}" != "1" ]; then
  ...
  CUDA_VISIBLE_DEVICES=${GPUS%%,*} ${CONDA_VENV}/python scripts/h100b/smoke_utonia_prompttoken.py \
    --mode fromscratch \
    --base-vlm "${base_vlm}" \
...
echo "[launch] BS=$BS x $NUM_PROCESSES GPU x GA${DS_GA} (${DS_CONFIG##*/}) = eff_$((BS*NUM_PROCESSES*DS_GA)) | MAX_STEPS=$MAX_STEPS | SAVE_INTERVAL=$SAVE_INTERVAL | WARMUP=$WARMUP_STEPS | run_id=$run_id"
