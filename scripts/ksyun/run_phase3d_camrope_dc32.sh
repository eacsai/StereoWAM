#!/bin/bash
# Phase 3 D: Stereo+CamRoPE d_c=32 ablation (intermediate point in d_c sweep 16/32/64).
...
run_id=${RUN_ID:-goal_phase3d_camrope_dc32_$(date +%m%d)}
...
echo "[launch] Phase 3 D Stereo+CamRoPE d_c=32 (intermediate))  run_id=${run_id}"
...
CUDA_VISIBLE_DEVICES=${GPUS} ${CONDA_VENV}/accelerate launch \
  ...
  --framework.qwenvl.stereo_cam_rope_d_c 32 \
