#!/usr/bin/env bash
# Runs ON 4090d. GPU-wait queue for #8 ValueResidual V branch, 2 arms:
#   fromscratch (ablation) then warmstartB (frozen trunk + zero-init Value residual).
# Mirrors method7; only job table, framework, allowlist, ports, launcher, and log differ.
set -uo pipefail
...
LOG=/data/wangqiwei/ICLR2026/starVLA/playground/Checkpoints/wait_method8.log
B_CKPT=
TRAIN_ONLY_ALLOW=".attn1.to_v_resid."
GPU_FREE_MB=4000
POLL=600
SETTLE=300
...
PORTS=( 29752 29753 )
KINDS=( vr_fromscratch vr_warmstartB )

log(){ echo "[m8-wait $(date -u +%FT%TZ)] $*" | tee -a "$LOG"; }
...
log "=== method8 ValueResidual GPU-wait launcher started (order: ${KINDS[*]}) ==="
for i in "${!RIDS[@]}"; do
...
log "=== method8 jobs launched/handled -- watcher exiting ==="
