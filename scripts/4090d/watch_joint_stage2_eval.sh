#!/usr/bin/env bash
# Keep-alive + auto-resume + eval watcher for the a800a GPU0 STAGE-2 (joint_cascade, warmstart from plain stage-1 flow ckpt).
# Adapted from watch_and_eval_cascade.sh: a800a endpoint + _resume_joint_stage2.sh + stage-2 run_id.
...
RUN_ID="${RUN_ID:-qwen0p8_groot_cascade_motion_cambranch_joint_warmstartflow_leftprimary}"
STEP="${STEP:-30000}"
...
relaunch_count=0
last_seen_step=0
mark "WATCHING stage-2 run=$RUN_ID on a800a GPU0 (keep-alive, poll ${POLL}s, max_relaunch=$MAX_RELAUNCH)"
...
while true; do
  ...
  if [ "$session" = "GONE" ] && [ "$train_exit" != "TRAIN_EXIT=0" ]; then
    ...
    if [ -n "$any_ckpt" ]; then
      $A800 "cd '$A800_REPO' && bash scripts/a800/_resume_joint_stage2.sh 1" >> "$LOG" 2>&1 || true
      mark "RESUMED (#$relaunch_count) hot-restart own ckpt (last_step=$last_seen_step)"
    else
      $A800 "cd '$A800_REPO' && bash scripts/a800/_resume_joint_stage2.sh 0" >> "$LOG" 2>&1 || true
