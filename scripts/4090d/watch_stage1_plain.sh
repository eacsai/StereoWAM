#!/usr/bin/env bash
# Keep-alive + auto-resume watcher for the a800a GPU0 PLAIN stage-1 (learn_scene_flow, flow_only).
...
relaunch_count=0
last_seen_step=0
mark "WATCHING plain stage-1 on a800a GPU0 (keep-alive, no-eval, poll ${POLL}s)"
...
while true; do
  ...
  if [ -n "${remote_sz:-}" ] && [ "${remote_sz:-0}" -gt 1000000 ]; then
    mark "STAGE1_DONE step=$STEP (learn_scene_flow flow_only complete on a800a). Ckpt at $CKPT_FINAL. Awaiting manual stage-2 decision (no auto-progression)."
    exit 0
  fi
  ...
  if [ "$session" = "GONE" ] && [ "$train_exit" != "TRAIN_EXIT=0" ]; then
    ...
    $A800A "cd '$A800_REPO' && bash scripts/a800/_resume_learnflow_plain.sh $R" >> "$LOG" 2>&1 || true
