...
class VLAMTrainer(TrainerUtils):
    ...
    def prepare_training(self):
        ...
        if hasattr(self.config.trainer, "pretrained_checkpoint") and self.config.trainer.pretrained_checkpoint:
            ...
            self.model = self.load_pretrained_backbones(
                ...
                init_from_baseline=self._scene_flow_enabled(),
            )
    ...
    def _train_step(self, batch_vla, batch_vlm):
            ...
            if self._scene_flow_enabled() and flow_loss is not None:
                ...
                for key in (
                    ...
                    if value is not None:
                        log_dict[f"scene_flow/{key}"] = value
                log_dict.update(grad_ratio_metrics)
                log_dict.update(audit_metrics)
        ...
        if self._scene_flow_online_grad_norm_enabled():
            log_dict.update(self._scene_flow_online_state_metrics())
            log_dict.update(online_gradnorm_metrics)
